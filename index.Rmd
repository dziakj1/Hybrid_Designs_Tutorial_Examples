...
used in analyzing the design (see Nahum-Shani et al., 2023, for details)
...
- There is a statistically significant beneficial main effect of the *Coaching* factor (p<.001).  Because the factor was effect-coded with yes represented as +1 and no as -1,
the main effect (average final outcome difference between the yes and no levels) 
is twice the regression coefficient, thus 2 * 0.7145, or about 1.4 more kilograms lost, which may also be clinically significant.  The *App* factor does not have a statistically significant main effect (p=.2893) so there is no evidence that the app is beneficial on average. There doesn't seem to be a significant interaction between *Coaching* and *App* (p=.3142), so we can make decisions based on their main effects alone.  It looks like there is reason to begin the intervention with Coaching, but not necessarily with App.

- The *Meal* factor for nonresponders has a statistically significant main effect (p<.001).  The main effect is twice 0.9270, or about 1.8 kilograms, which may be clinically significant. There is also a beneficial interaction (possible synergy) between *Coaching* and *Meal*.

Biological sex (coded +1 for female and -1 for male), and baseline BMI, also centered at 0, are  included as covariates in the example analysis, mainly to account for extra error variance, but their coefficients are not considered directly relevant to the research questions.  Nonetheless, higher BMI seems to be associated with lower weight loss (p<.001) but sex does not seem to matter.

Note that, of course, this is simulated data from a hypothetical study, so it should not be considered for making real-world conclusions.  It is only provided as a template for planning and analyzing future real studies.  Also, note that this example does not contain information on costs, which would also be an issue in deploying the proposed intervention in the real world (especially because meal replacement can be expensive).  Cost-effectiveness would be a separate analysis.

***
 
# Hybrid Factorial-MRT
         
In the example hybrid factorial-MRT,  the intervention being considered the initial intervention would include a supportive app that can be used to record dietary intake, and it might include coaching sessions (*Coaching*) and/or meal replacement (*Meal*), which would be offered at the beginning and continue throughout the study. Also, during the course of the study, a mobile phone text prompt or push notification (*Prompt*) might be given on some days at noon, reminding the participant to monitor food intake.  The researcher wants to know whether this prompt improves the short-term goal or 'proximal outcome' of dietary self-monitoring that day. Thus there are separate questions, requiring separate analyses, for the  distal and proximal outcomes. 

The researcher wants to investigate the following questions:
- Are either or both of *Coaching* and *Meal*  sufficiently effective to be worth including at the beginning, in terms of the final goal or 'distal outcome' of weight loss after six months?
- Is *Prompt* effective, on the day when it is given, in improving the probability that the participant will engage with dietary tracking?
...
We need to fit separate models for the distal and the proximal outcome.
The distal outcome is observed on the person-level; it happens once per person.
In contrast, the proximal outcome happens once per day per person.
...
We don't test the effect of an individual occasion-level prompt (e.g., on a particular day) on the distal outcome, because that would likely be too small to detect.  The hypothetical study isn't designed for comparing prompts versus no prompts on the distal outcome, because in this hypothetical design everyone got prompts about half the time; the main difference was which days.  So whether prompts are ever helpful in this intervention for this population is still unknown.  They do not at least seem to affect the proximal outcome very much. 
...
Again, we need to fit separate models for the distal and the proximal outcome. The output looks very similar to the factorial-MRT example, which is not surprising because a SMART is a special kind of factorial design. However, the analysis for the SMART was different: it took into account the fact that not only nonresponders were randomized to a level of the *meal* factor. 
...
It does have a significant and positive interaction with coaching, however (p=0.030).  Therefore, if coaching is being provided, then prompts may also be useful. 
